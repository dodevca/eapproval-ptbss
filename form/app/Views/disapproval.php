...
<html>
    <head>
        ...
        <title><?= 'Disapproval ' . ucwords($type) . ' - ' . $document['proofNumber'] ?></title>
    ...
    <body class="bg-light">
        ...
        <main>
            <div class="container-fluid <?= $message == 'Disapproved.' || $message == 'Notes submitted.' ? 'bg-success' : 'bg-warning'; ?> text-center py-4">
                <h1 class="h3 mb-0 text-center text-white"><?= $message == 'Disapproved.' || $message == 'Notes submitted.' ? $message == 'Notes submitted.' ? 'Catatan Anda Berhasil Dikirim' : 'Respon Disapprove Anda Berhasil Dikirim' : 'Respon Anda Gagal Dikirim'; ?></h1>
            </div>
            <div class="container py-4">
                <?php if (($message == 'Disapproved.' && $prevApprover == null) || ($message == 'Notes submitted.' && $prevApprover == null)): ?>
                    <h2 class="h4 text-center">Terima kasih, atas respon anda.</h2>
                    <p class="text-center"><?= $message == 'Notes submitted.' ? 'Catatan telah dikirimkan ke pemohon.' : 'Dokumen ini selanjutnya akan dikembalikan ke Pemohon dan dihapus.' ?></p>
                <?php elseif (($message == 'Disapproved.' && $prevApprover != null) || ($message == 'Notes submitted.' && $prevApprover != null)): ?>
                    <h2 class="h4 text-center">Terima kasih, atas respon anda.</h2>
                    <p class="text-center"><?= $message == 'Notes submitted.' ? 'Catatan telah dikirimkan ke <u>' . str_replace('_', ' ', $prevApprover) . '</u>.' : 'Dokumen ini selanjutnya akan dikembalikan ke <u>' . str_replace('_', ' ', $prevApprover) . '</u> untuk dicek kembali.' ?></p>
                <?php else: ?>
                    <h2 class="h4 text-center">Mohon maaf, dokumen telah direspon.</h2>
                    <p class="text-center">Dokumen ini tekah dikembalikan ke <u><?= str_replace('_', ' ', $prevApprover) ?></u>.</p>
                <?php endif; ?>
                <?php if (($message == 'Disapproved.' && $notes == null) || ($message == "Can't disapprove now." && $notes == null)): ?>
                    <div class="border p-3 rounded mb-4 mx-auto" style="max-width:1220px">
                        <h3 class="mb-0">Masukkan Catatan</h3>
                        <?php if ($message == "Can't disapprove now." && $approver['name'] == $disapproval && $notes == null): ?>
                            <h6 class="text-danger">Anda belum mengirimkan catatan untuk <?= str_replace('_', ' ', $prevApprover) ?>!</h6>
                        <?php else: ?>
                            <p>Berikan catatan kepada <?= str_replace('_', ' ', $prevApprover) ?>!</p>
                        <?php endif; ?>
                        <?= form_open(base_url('/submit/notes')); ?>
                            <input type="text" class="d-none" name="as" value="<?= $disapproved['hierarchy'] ?>" required />
                            <input type="text" class="d-none" name="doc" value="<?= $file['submission'] ?>" required />
                            <div class="mb-3">
